...
// axios.defaults.baseURL = 'http://111.231.59.56:5000/api' //请求地址的域名（此处使用了代理所以直接填／apis
...
const service = axios.create({
    baseURL:'http://111.231.59.56:5000/api',//process.env.VUE_APP_BASE_API , //process.env.VUE_APP_BASE_LOCAL_API, api的base_url 
    timeout: 30000 
})
...
service.interceptors.request.use(config =>{
    ...
    // config.headers['Abp.TenantId'] = USER_INFO.tenantId || null;
    return config
},error => {
    return Promise.reject(error)
})
